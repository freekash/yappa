 ...
 *
 * @return string
 * render single post featured layout 7
 */
if ( ! function_exists( 'newsmax_ruby_render_single_post_featured_layout_7' ) ) {
	function newsmax_ruby_render_single_post_featured_layout_7( $param ) {
		...
		$class_name   = array();
		$class_name[] = 'single-post-wrap single-post-7 is-single-' . esc_attr( $header_position );
		if ( ! empty( $parallax ) ) {
		...
		$str .= '<div class="single-post-feat-screen-outer">';
		$str .= '<div class="single-post-feat-screen"></div>';

		$param['thumb_size'] = 'full';
