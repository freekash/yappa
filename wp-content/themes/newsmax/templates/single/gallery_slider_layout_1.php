 ...
 *
 * @return string
 * render single post gallery slider
 */
if ( ! function_exists( 'newsmax_ruby_render_single_post_gallery_slider_layout_1' ) ) {
	function newsmax_ruby_render_single_post_gallery_slider_layout_1( $param = array() ) {
		...
		$class_name                = 'single-post-wrap single-post-1 single-post-gallery-slider-1 is-single-' . esc_attr( $header_position );
		$author_box                = newsmax_ruby_get_option( 'single_post_box_author' );
		$ajax_view                 = newsmax_ruby_get_option( 'ajax_view' );
		$param['sidebar_position'] = $sidebar_position;
		$param['gallery_style']    = 1;
		...
		$str .= newsmax_ruby_single_post_thumb_gallery_slider( $param );
