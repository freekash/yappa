...
/**-------------------------------------------------------------------------------------------------------------------------
 * @return string
 * newsmax_ruby_post_feat_6
 */
if ( ! function_exists( 'newsmax_ruby_post_feat_6' ) ) {
	function newsmax_ruby_post_feat_6( $options = array() ) {

		$review = newsmax_ruby_post_review_icon( 'is-size-2 is-absolute' );

		$str = '';
		$str .= '<article class="post-wrap post-feat post-feat-6">';
		...
		if ( has_post_thumbnail() ) {
			...
			$param['size_mobile_h'] = 'newsmax_ruby_crop_380x380';
