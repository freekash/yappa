<?php
/**
 * Class newsmax_ruby_hs_block_grid_6
 */
if ( ! class_exists( 'newsmax_ruby_hs_block_grid_6' ) ) {
	class newsmax_ruby_hs_block_grid_6 extends newsmax_ruby_block {
		...
		static function render( $block ) {
			...
			$block['block_classes'] = 'hs-block hs-block-grid-6';
			if ( ! empty( $block['block_options']['grid_style'] ) ) {
				$block['block_classes'] = 'hs-block hs-block-grid-6 is-grid-style-' . esc_attr( $block['block_options']['grid_style'] );
			}
		...
		static function listing( $data_query, $options = array() ) {
			...
			while ( $data_query->have_posts() ) {
				...
				$str .= '<div class="col-sm-6 col-xs-12 post-outer-nth-' . esc_attr( $counter_style ) . ' post-outer">';
