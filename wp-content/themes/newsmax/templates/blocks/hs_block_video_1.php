<?php
/**
 * Class newsmax_ruby_hs_block_video_1 (video playlist)
 */
if ( ! class_exists( 'newsmax_ruby_hs_block_video_1' ) ) {
	class newsmax_ruby_hs_block_video_1 extends newsmax_ruby_block {
		...
		static function render( $block ) {
			...
			$block['block_classes'] = 'hs-block hs-block-video hs-block-video-1';
		...
		static function listing( $data_query, $options = array() ) {
			...
			while ( $data_query->have_posts() ) {
				$data_query->the_post();
				$str .= '<div class="post-outer col-sm-4 col-xs-12 video-playlist-iframe-nav-el is-light-text" data-post_video_nav_id="' . get_the_ID() . '">';
