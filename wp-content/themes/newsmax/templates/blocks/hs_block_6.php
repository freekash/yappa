...
/**
 * Class newsmax_ruby_hs_block_6
 */
if ( ! class_exists( 'newsmax_ruby_hs_block_6' ) ) {
	class newsmax_ruby_hs_block_6 extends newsmax_ruby_block {
		...
		static function render( $block ) {
			...
			$block['block_classes'] = 'hs-block block-default hs-block-6';
		...
		static function listing( $data_query, $options = array() ) {
			...
			while ( $data_query->have_posts() ) {
				...
				if ( 1 == $counter ) {
					...
					$str .= newsmax_ruby_post_overlay_2( $options );
					$str .= '</div><!--#left column-->';
				} else {
					if ( 2 == $counter ) {
						$str .= '<div class="col-right col-sm-4 col-xs-12">';
					}
					$str .= '<div class="post-outer col-sx-12">';
