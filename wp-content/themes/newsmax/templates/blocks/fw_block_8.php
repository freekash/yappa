<?php
/**
 * Class newsmax_ruby_fw_block_8
 */
if ( ! class_exists( 'newsmax_ruby_fw_block_8' ) ) {
	class newsmax_ruby_fw_block_8 extends newsmax_ruby_block {
		...
		static function render( $block ) {
			...
			$block['block_classes']                  = 'fw-block block-feat fw-block-8';
			$block['block_options']['no_found_rows'] = true;
			if ( ! empty( $block['block_options']['grid_style'] ) ) {
				$block['block_classes'] = 'fw-block block-feat fw-block-8 is-grid-style-' . esc_attr( $block['block_options']['grid_style'] );
			}
			...
			$block['block_options']['posts_per_page'] = $block['block_options']['slides_per_page'] * 6;
		...
		static function listing( $data_query, $options = array() ) {
			...
			$loop          = intval( floor( $total / 6 ) );

			if ( 1 > $loop ) {
				return parent::not_enough_post( 6 );
			}

			if ( 1 < $loop ) {
				$str .= '<div class="fw-block-8-slider-outer">';
				$str .= '<div class="slider-loader"></div>';
				$str .= '<div class="fw-block-8-slider slider-init">';
			}

			for ( $i = 1; $i <= $loop; $i ++ ) {
				$str .= '<div class="fw-block-8-slider-el">';
