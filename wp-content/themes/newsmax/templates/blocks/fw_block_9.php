<?php
/**
 * Class newsmax_ruby_fw_block_9
 */
if ( ! class_exists( 'newsmax_ruby_fw_block_9' ) ) {
	class newsmax_ruby_fw_block_9 extends newsmax_ruby_block {
		...
		static function render( $block ) {
			...
			$block['block_classes']                  = 'fw-block block-feat fw-block-9';
			$block['block_options']['no_found_rows'] = true;
			if ( ! empty( $block['block_options']['grid_style'] ) ) {
				$block['block_classes'] = 'fw-block block-feat fw-block-9 is-grid-style-' . esc_attr( $block['block_options']['grid_style'] );
			}
			...
			$block['block_options']['posts_per_page'] = $block['block_options']['slides_per_page'] * 7;
		...
		static function listing( $data_query, $options = array() ) {
			...
			$loop          = intval( floor( $total / 7 ) );

			if ( 1 > $loop ) {
				return parent::not_enough_post( 7 );
			}

			if ( 1 < $loop ) {
				$str .= '<div class="fw-block-9-slider-outer">';
				$str .= '<div class="slider-loader"></div>';
				$str .= '<div class="fw-block-9-slider slider-init">';
			}

			for ( $i = 1; $i <= $loop; $i ++ ) {
				$str .= '<div class="fw-block-9-slider-el">';
				while ( $data_query->have_posts() ) {
					$data_query->the_post();

					if ( 1 == $counter || 2 == $counter || 3 == $counter ) {
						$str .= '<div class="post-outer post-outer-nth-' . esc_attr( $counter_style ) . ' col-sm-4 col-xs-12">';
						$str .= newsmax_ruby_post_feat_4( $options );
						$str .= '</div>';
					} else {
						$str .= '<div class="post-outer post-outer-nth-' . esc_attr( $counter_style ) . ' col-sm-3 col-xs-6">';
						$str .= newsmax_ruby_post_feat_7( $options );
						$str .= '</div>';
					}
