...
if ( ! class_exists( 'RW_Taxonomy_Meta' ) ) {
	class RW_Taxonomy_Meta {
		 ...
		 */
		function output_css() {
			echo esc_html($this->css) ? '<style>' . $this->css . '</style>' : '';
		}
		 ...
		 */
		function output_js() {
			echo esc_html($this->js) ? '<script>jQuery(function($){' . $this->js . '});</script>' : '';
		}
		...
		';
			...
			\$('body').on('click', '.rwtm-file-upload', function(){
				...
				template += ' (<a class=\"rwtm-delete-file\" href=\"#\">" . esc_html__( 'Delete', 'newsmax-core' ) . "</a>)';
				...
				var frame = wp.media({
					multiple : true,
					title    : \"" . esc_html__( 'Select File', 'newsmax-core' ) . "\"
				});
		...
		\$('body').on('click', '.rwtm-image-upload', function(){
			...
			var frame = wp.media({
				multiple : true,
				title    : \"" . esc_html__( 'Select Image', 'newsmax-core' ) . "\",
				library  : {
					type: 'image'
				}
			});
		...
		function show_field_end( $field, $meta ) {
			echo esc_html($field['desc']) ? "<br><span class='description'>{$field['desc']}</span></td>" : '</td>';
		}
		...
		function show_field_file( $field, $meta ) {
			...
			foreach ( $meta as $att ) {
				...
				<li>
					...
					esc_html__( 'Delete', 'newsmax-core' ),
					$field['id'],
					$att
				);
			}
			...
			echo "<a href='#' class='rwtm-file-upload button' data-field='{$field['id']}'>" . esc_html__( 'Select File', 'newsmax-core' ) . "</a>";
			echo '</td>';
		}
		...
		function show_field_image( $field, $meta ) {
			...
			echo "<a href='#' class='rwtm-image-upload button' data-field='{$field['id']}'>" . esc_html__( 'Select Image', 'newsmax-core' ) . "</a>";
			echo '</td>';
		}
